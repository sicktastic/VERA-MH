...
class TestConversationSimulator:
    ...
    async def test_max_total_words_stopping_condition(self):
        ...
        # Arrange - Use agent named "Provider" to trigger the max_total_words check
        persona = MockLLM(
            name="User",
        ...
        # Turn 2: Provider says "I am doing well today" (5 words, total: 7)
        # Turn 3: User says "How are you" (3 words, total: 10)
        # Turn 4: Provider says "Very good indeed" (3 words, total: 13)
        # Should stop after turn 4 since Provider exceeded max_total_words
        assert len(history) == 4
    ...
    async def test_max_total_words_only_stops_after_chatbot_turn(self):
        """Test that max_total_words only checks after Provider (agent) speaks."""
        # Arrange
        persona = MockLLM(
            name="User",
        ...
        # Act - Even though User exceeds limit, should only stop after Provider
